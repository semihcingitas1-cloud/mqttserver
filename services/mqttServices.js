...
const MQTT_BROKER = "mqtt://10.210.237.8:1883";
...
const sendCommand = (device, command) => {
  ...
  client.publish(topic, payload, { qos: 1 }, (err) => {
    if (err) {
      console.error(`❌ Publish hatası (${topic}):`, err.message);
    } else {
      console.log(`✅ Komut başarıyla gönderildi → ${device.serialNumber} / ${command}`);
    }
  });
};
